...
import DependenciesTabComponent from '../../../../../src/components/service-window/DependenciesTabComponent.vue';
...
describe('dependency tab component', () => {

  it('should load [after loading]', async () => {
    const wrapper = mount(DependenciesTabComponent, {
      props: {
        service: service,
        host: {
          username: 'user',
    ...
    await flushPromises();
    await nextTick();
    expect(wrapper.html()).toContain('play_arrow');
  });
});
